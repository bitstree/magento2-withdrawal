...
namespace BitsTree\Withdrawal\Block\Order;
...
/**
 * Withdrawal button block for order detail view
 */
class WithdrawalButton extends Template
{
    ...
    public function canShowButton(?OrderInterface $order = null): bool
    {
        ...
        if (!$this->config->isShowOnOrderView($storeId)) {
            return false;
        }
    ...
    public function getEligibleItems(): array
    {
        ...
        return $this->config->getEligibleItems($order, (int) $order->getStoreId());
    }
    ...
    {
        $order = $this->getOrder();
        return $this->getUrl('withdrawal/index/submit', ['order_id' => $order ? $order->getEntityId() : 0]);
    }
